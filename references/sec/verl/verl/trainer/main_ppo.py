...
class RewardManager():
    ...
    def __init__(self, tokenizer, num_examine, correct_reward, format_reward) -> None:
    ...
    def __call__(self, data: DataProto):
        ...
        def process_item(args):
            ...
            compute_score_fn = _select_rm_score_fn(data_source, self.correct_reward, self.format_reward)
...
def main_task(config):
    ...
    reward_fn = RewardManager(tokenizer=tokenizer, num_examine=0, correct_reward=config.reward_model.reward, format_reward=config.reward_model.format_reward)

    # Note that we always use function-based RM for validation
    val_reward_fn = RewardManager(tokenizer=tokenizer, num_examine=1, correct_reward=1.0, format_reward=0.0)
