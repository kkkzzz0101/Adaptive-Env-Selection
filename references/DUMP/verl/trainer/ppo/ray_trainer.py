...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        if enable_curriculum_learning and not data_source_key:
            print("Warning: Curriculum learning is enabled but data_source_key is not specified. Disabling curriculum learning.")
        ...
        self.train_dataset = RLHFDataset(parquet_files=self.config.data.train_files,
                                         ...
                                         run_diagnostics=enable_curriculum_learning)  # Run diagnostics when curriculum learning is enabled
        ...
        # For validation dataset, we don't need curriculum learning
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # For curriculum learning: collect rewards and compute advantages by data source
        ...
        #print("reward_extra_info_dict: ", reward_extra_info_dict)
        if reward_extra_info_dict is not None:
            #print(f"Extra info keys: {list(reward_extra_info_dict.keys())}")
            #print(f"Data sources in this validation: {set(data_sources)}")
        ...
        #print("data_source_reward_extra_info: ", data_source_reward_extra_info)
        if reward_extra_info_dict is not None:
    ...
    def save_and_upload_checkpoint(self, upload_to_hf=False, hf_repo_id=None):
        ...
        if upload_to_hf:
            ...
            if not self.config.trainer.get("hf_account", None) and not hf_repo_id:
                print("Skipping HF upload: trainer.hf_account is not set.")
                return
            merger_script = os.path.join(os.getcwd(), "scripts", "model_merger.py")
            if not os.path.exists(merger_script):
                print("Skipping HF upload: merger script not found at " + merger_script)
                return
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if hasattr(self, 'curriculum_sampler') and self.curriculum_sampler is not None:
                    ...
                    source_stats = self.curriculum_sampler.get_source_stats()
                    ...
                    for source, stats in source_stats.items():
                        ...
                        metrics[f'curriculum/sample_count/{source}'] = stats.get('n_samples', 0.0)
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        # [AES-DEBUG] Optional smoke-test probe for prompt/response/reward/advantage.
                        debug_every = int(os.environ.get('AES_DEBUG_SAMPLE_EVERY', '0') or 0)
                        if debug_every > 0 and (self.global_steps % debug_every == 0):
                            try:
                                debug_count = int(os.environ.get('AES_DEBUG_SAMPLE_COUNT', '2') or 2)
                                rewards_for_print = batch.batch.get('token_level_rewards', None)
                                if rewards_for_print is None:
                                    rewards_for_print = batch.batch['token_level_scores']
                                adv_for_print = batch.batch['advantages']
                                ...
                                uid_to_indices = {}
                                if uids is not None:
                                    for i_uid, uid in enumerate(uids):
                                        uid_str = str(uid)
                                        if uid_str not in uid_to_indices:
                                            uid_to_indices[uid_str] = []
                                        uid_to_indices[uid_str].append(i_uid)

                                bs = len(batch.batch)
                                num_pick = min(debug_count, bs)
                                picked = np.random.choice(bs, size=num_pick, replace=False).tolist()

                                print(f"[AES-DEBUG] step={self.global_steps} batch_size={bs} sample_count={num_pick}")
                                for pi, idx in enumerate(picked):
                                    prompt_text = '<prompt_unavailable>'
                                    if 'prompts' in batch.batch:
                                        ...
                                        prompt_ids = batch.batch['input_ids'][idx]
                                        if hasattr(prompt_ids, 'detach'):
                                            prompt_ids = prompt_ids.detach().cpu().tolist()
                                        prompt_text = self.tokenizer.decode(prompt_ids, skip_special_tokens=True)

                                    response_text = '<response_unavailable>'
                                    if 'responses' in batch.batch:
                                        response_ids = batch.batch['responses'][idx]
                                        if hasattr(response_ids, 'detach'):
                                            response_ids = response_ids.detach().cpu().tolist()
                                        response_text = self.tokenizer.decode(response_ids, skip_special_tokens=True)

                                    reward_sum = rewards_for_print[idx].sum().item()
                                    if response_mask is not None and response_mask[idx].any():
                                        adv_mean = adv_for_print[idx][response_mask[idx]].mean().item()
                                    else:
                                        adv_mean = adv_for_print[idx].mean().item()

                                    uid_str = str(uids[idx]) if uids is not None else f'idx_{idx}'
                                    group_rewards = []
                                    group_advs = []
                                    if uid_str in uid_to_indices:
                                        for gj in uid_to_indices[uid_str]:
                                            group_rewards.append(float(rewards_for_print[gj].sum().item()))
                                            if response_mask is not None and response_mask[gj].any():
                                                group_advs.append(float(adv_for_print[gj][response_mask[gj]].mean().item()))
                                            else:
                                                group_advs.append(float(adv_for_print[gj].mean().item()))

                                    prompt_one_line = prompt_text.replace('\n', ' ')[:1200]
                                    response_one_line = response_text.replace('\n', ' ')[:1200]
                                    print(f"[AES-DEBUG][sample {pi+1}] uid={uid_str} reward={reward_sum:.4f} adv={adv_mean:.4f} group_rewards={group_rewards} group_advantages={group_advs}")
                                    print(f"[AES-DEBUG][sample {pi+1}] problem={prompt_one_line}")
                                    print(f"[AES-DEBUG][sample {pi+1}] response={response_one_line}")
                            except Exception as debug_e:
                                print(f"[AES-DEBUG] print_failed step={self.global_steps}: {debug_e}")

                        # Update curriculum weights/state using batch data
                        if hasattr(self, 'curriculum_sampler') and self.curriculum_sampler is not None:
                            data_source_rewards = {}
                            data_source_advantages = {}
                            sample_ids = []
                            sample_abs_advantages = []

                            current_data_sources = batch.non_tensor_batch.get('data_source', ['unknown'] * len(batch.batch['token_level_rewards']))
                            current_sample_ids = batch.non_tensor_batch.get('sample_id', None)
                            ...
                            for i, source in enumerate(current_data_sources):
                                ...
                                if response_mask is not None and response_mask[i].any():
                                    sample_adv = advantages[i][response_mask[i]].mean().item()
                                else:
                                    sample_adv = advantages[i].mean().item()

                                data_source_rewards[source].append(sample_rewards)
                                data_source_advantages[source].append(sample_adv)

                                if current_sample_ids is not None and i < len(current_sample_ids):
                                    sid = str(current_sample_ids[i])
                                else:
                                    sid = f'unknown_{self.global_steps}_{i}'
                                sample_ids.append(sid)
                                sample_abs_advantages.append(abs(sample_adv))

                            # Prefer sample-level scheduler updates when available.
                            if hasattr(self.curriculum_sampler, 'update_sample_states'):
                                self.curriculum_sampler.update_sample_states(
                                    sample_ids=sample_ids,
                                    sample_advantages=sample_abs_advantages,
                                    global_step=self.global_steps,
                                )
                            elif data_source_rewards and data_source_advantages:
                    ...
                    # validate
                    if self.val_reward_fn is not None and not self.config.trainer.get('skip_final_validation', False) and self.config.trainer.test_freq > 0 and \
                ...
                if self.global_steps >= self.total_training_steps:
                    ...
                    if self.val_reward_fn is not None and not self.config.trainer.get('skip_final_validation', False):
                        val_metrics = self._validate()
